...
server=gradio_client.Client(load_env_var('AGG_SERVER_ADDR','str'))
    ...
    while(c_model.model_ver<=i):
        trainer=L.Trainer(
            callbacks=[
                ...
                Timer(duration=TRAIN_TIME,interval="step")
                ],
        ...
        torch.save(c_model.state_dict(),f'/workspace/{c_model.model_ver}.pth')
        server.predict(f'/workspace/{c_model.model_ver}.pth',trainer.global_step,api_name='/upload_model')
    else:
